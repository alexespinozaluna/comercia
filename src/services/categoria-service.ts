...
export const categoriaService = {
  ...
  async create(
    ...
    const { data, error } = await getSupabaseServer()
      .from(TABLE)
      .insert(
        auditCreate(idUsuario, { IdTenant: tenantId, Nombre: limpio, Estado: 1 }),
      )
      .select()
      .single();
  ...
  async rename(
    ...
    const { error } = await getSupabaseServer()
      .from(TABLE)
      .update(auditUpdate(idUsuario, { Nombre: limpio }))
      .eq("id", id)
   ...
   */
  async remove(id: number, tenantId: number, idUsuario: number): Promise<void> {
    ...
    const { error: prodErr } = await supabase
      .from("Producto")
      .update(auditUpdate(idUsuario, { IdCategoria: SIN_CATEGORIA_ID }))
      .eq("IdCategoria", id)
      .eq("IdTenant", tenantId);
    ...
    const { error } = await supabase
      .from(TABLE)
      .update(auditUpdate(idUsuario, { Estado: 0 }))
      .eq("id", id)
      .eq("IdTenant", tenantId);
