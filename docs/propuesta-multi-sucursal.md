...
3. **Escrituras + RPC/triggers + lecturas por sucursal** — la parte crítica y de mayor riesgo. En sub-pasos:
   - **3a — escrituras pueblan `IdNegocio`** ✅ implementada: migración `20260528020000_multi_sucursal_fase3a_escrituras.sql` (RPCs `guardar_venta_con_items` y `registrar_abono` con `p_id_negocio`; el abono hereda el negocio de la deuda y el FIFO se acota a la sucursal; trigger de stock pone `ProductoMovimiento.IdNegocio`; índice único de caja pasa a `(IdTenant, IdNegocio)`) + TS (`caja-service` `getCajaAbierta`/`abrirCaja` negocio-aware, `documento-service` pasa `p_id_negocio`, rutas ventas/abonos/gastos/ajustes/caja envían `user.idNegocio`, gastos/ajustes ponen `IdNegocio` en sus inserts directos). NO cambia el origen del stock ni filtra lecturas. Compatibilidad: con `idNegocio` NULL (token previo) no filtra.
   - **3b — cutover de stock a `ProductoStock`** ✅ implementada: migración `20260528040000_multi_sucursal_fase3b_stock.sql` (trigger `fn_registrar_movimiento_stock` lee/escribe `ProductoStock(IdProducto, IdNegocio)` con upsert, ya no toca `Producto.Cantidad`). TS: `producto-service.getAll/getById` sobrescriben `Cantidad` con el stock de la sucursal activa; `ajustes` lee (vía `getById` con sucursal) y escribe en `ProductoStock`; alta de producto siembra `ProductoStock` + movimiento con `IdNegocio`. `Producto.Cantidad` queda como bandera "rastrea stock" (null=no) + legacy. Fallback a `Producto.Cantidad` si la sesión no trae sucursal.
   - **3c — filtrar lecturas por sucursal** ✅ implementada: migración `20260528030000_multi_sucursal_fase3c_lecturas.sql` (re-backfill de NULLs del intervalo + `v_deuda_detalle` expone `IdNegocio` + `fn_deuda_resumen(p_id_tenant, p_id_negocio)`). TS: `getVentas`/`getVentaConItem`/`getVentasEliminadas`/`getDeudaDetalle`/`getDeudaResumen`, `caja getHistorial`, `kardex getByProducto` filtran por `user.idNegocio` (guard null = nivel cuenta). El link público de deuda NO filtra (agrega todas las sucursales del cliente). El selector, al cambiar, navega a `/` y refresca; la home refetchea por `refreshCounter`.
   - **3d — re-aplicar `NOT NULL`** (pendiente) en `Documento`/`DocumentoItem`/`Caja`/`ProductoMovimiento`. ⚠️ **Riesgo:** un token previo (sin `idNegocio`) haría que las escrituras pongan `IdNegocio` NULL → violación. Antes de 3d: garantizar que todas las sesiones re-iniciaron, o hacer que las RPC/inserts hagan `COALESCE(idNegocio, principal del tenant)`. El re-backfill de NULLs ya se hizo en 3c.
